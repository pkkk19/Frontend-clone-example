...
import '../../assets/scss/page/project-access-info.scss';
...
export default function ProjectAccessInfo({ next }) {
  ...
  function checkAccess(data) {
    const newList = list.map((item) => {
      ...
      let systemMajor = 100;
      let systemMinor = 100;
      let minimumMajor = 100;
      let minimumMinor = 100;

      try {
        systemMajor = Number(system[0]);
        systemMinor = Number(system[1]);
        minimumMajor = Number(minimum[0]);
        minimumMinor = Number(minimum[1]);
      } catch (error) {
        console.log(error);
      }
